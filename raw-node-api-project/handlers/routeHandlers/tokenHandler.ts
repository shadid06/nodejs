...
export const tokenHandler = (requestProperties: RequestProperties, callback: (statusCode: number | undefined, payload: object | undefined) => void): void => {
    ...
    data.read("users", phone, (err, user) => {
        if(!err && user){
            ...
            if(hashedPassword === utilities.parseJSON(user).password){
                const tokenId:any = utilities.createRandomString(20);
...
export const verifyToken = (id: string, phone: string, callback: (isValid: boolean) => void) => {
    data.read("tokens", id, (err, tokenData) => {
        if (!err && tokenData) {
            const tokenObject = utilities.parseJSON(tokenData);
